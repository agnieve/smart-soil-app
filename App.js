...
export default function App() {
    ...
    useEffect(() => {
        ...
        (async ()=> {
            ...
            if(new Date(res3).getTime() > new Date(res3).getTime()){
                console.log("You cannot view success rate in crafts while the previous craft is not yet harvested");
                ...
                console.log("there was an errror in fetching the data. Please try again later.");
                setLoading(false);
  ...
  return (
    <View className="flex-1">
        {
            loading ? <View className={'flex flex-1 items-center justify-center w-full h-screen'}>
                <Image source={require('./assets/logo.png')}  className={'w-44 h-44 mb-3'} />
                </View> : <>
